...
CARD_FLIP_SOUND = os.path.join(_script_dir, "../assets/yinxiao.mp3")
...
def generate_image_background(fortune_data, api_key, forced_type: str, user_sentence: str = ""):
    try:
        ...
        image_prompt = (
            f"为用户生成一幅个人签运/祝福意象插画，画面必须高度个性化，严禁套用固定生肖、固定马年、新春、春节、财神、灯笼、红包模板。"
...
def generate_tts_audio(fortune_data, tts_api_key):
    if not tts_api_key:
        print("❌ 缺少 OPEN_SPEECH_X_API_KEY，无法生成视频")
        return None
...
def save_final_html(data, image_data_uri, has_video=False, video_base64="", filename=None):
    ...
    glow_color = type_glow_color(data.get("type", ""))
...
.card {{
  ...
  padding: 8px;
  box-shadow:
    0 18px 42px rgba(70,0,0,0.62),
    0 0 0 1px color-mix(in srgb, var(--glow) 68%, transparent),
    0 0 34px color-mix(in srgb, var(--glow) 62%, transparent);
  position: relative;
  transform-style: preserve-3d;
  animation: cardDraw 3.2s cubic-bezier(.2,.9,.12,1) both, glowPulse 2.4s ease-in-out 3.1s infinite;
}}
...
.card::before {{
  ...
  inset: -4px;
  border-radius: 22px;
  border: 2px solid color-mix(in srgb, var(--glow) 78%, transparent);
  filter: blur(1px);
  opacity: 0;
  animation: edgeIgnite 2.1s ease-out 1.65s both, edgeFlow 2.8s linear 3.5s infinite;
  pointer-events: none;
}}
...
.card::after {{
  ...
  inset: -18px;
  border-radius: 30px;
  background: radial-gradient(circle, color-mix(in srgb, var(--glow) 34%, transparent), transparent 62%);
  opacity: 0;
  filter: blur(12px);
  animation: auraBloom 1.8s ease-out 2.05s both;
  pointer-events: none;
}}
...
.card-inner {{
  ...
  z-index: 1;
  overflow: hidden;
  animation: resultReveal .9s ease-out 2.35s both;
}}
...
.card-inner::before {{
  ...
  position: absolute;
  inset: 0;
  background: linear-gradient(115deg, transparent 0%, rgba(255,255,255,.72) 46%, transparent 58%);
  transform: translateX(-130%);
  animation: shineSweep 1.25s ease-out 2.15s both;
  pointer-events: none;
}}
...
@keyframes bgDrift {{
  0% {{ transform: scale(1.08) translate3d(-18px, -10px, 0); filter: blur(34px); }}
  45% {{ transform: scale(1.13) translate3d(18px, 8px, 0); filter: blur(30px); }}
  100% {{ transform: scale(1.1) translate3d(0, 0, 0); filter: blur(28px); }}
}}

@keyframes cardDraw {{
  0% {{ opacity: 0; transform: translate3d(0, 46px, 0) scale(.78) rotateY(-104deg); }}
  18% {{ opacity: 1; transform: translate3d(0, 20px, 0) scale(.9) rotateY(-72deg); }}
  48% {{ opacity: 1; transform: translate3d(0, -4px, 0) scale(1.04) rotateY(14deg); }}
  68% {{ transform: translate3d(0, 0, 0) scale(.99) rotateY(-4deg); }}
  100% {{ opacity: 1; transform: translate3d(0, 0, 0) scale(1) rotateY(0); }}
}}

@keyframes resultReveal {{
  0% {{ opacity: .18; filter: blur(8px); }}
...
@keyframes edgeFlow {{
  0%, 100% {{ box-shadow: 0 0 12px var(--glow), inset 0 0 10px color-mix(in srgb, var(--glow) 40%, transparent); }}
  50% {{ box-shadow: 0 0 30px var(--glow), inset 0 0 18px color-mix(in srgb, var(--glow) 64%, transparent); }}
}}
...
@keyframes glowPulse {{
  0%, 100% {{ box-shadow: 0 18px 42px rgba(70,0,0,0.62), 0 0 0 1px color-mix(in srgb, var(--glow) 58%, transparent), 0 0 28px color-mix(in srgb, var(--glow) 45%, transparent); }}
  50% {{ box-shadow: 0 22px 52px rgba(70,0,0,0.68), 0 0 0 1px color-mix(in srgb, var(--glow) 88%, transparent), 0 0 48px color-mix(in srgb, var(--glow) 72%, transparent); }}
}}
</style>
</head>
  ...
  <div class="container">
    <div class="card">
      <div class="card-inner">
        ...
        <div class="user-signature">收福人：{data.get('user_name','')}</div>
...
def mix_audio_tracks(tts_mp3_path, bgm_path, effect_path, mixed_out_path):
    try:
        import imageio_ffmpeg
        ffmpeg_exe = imageio_ffmpeg.get_ffmpeg_exe()
    except Exception:
    ...
    cmd = [ffmpeg_exe, "-y", "-i", tts_mp3_path]
    filters = []
    labels = ["[0:a]"]
    input_index = 1
    ...
    if bgm_path and os.path.exists(bgm_path):
        ...
        filters.append(f"[{input_index}:a]volume={BGM_VOLUME}[bg]")
        labels.append("[bg]")
        input_index += 1

    if effect_path and os.path.exists(effect_path):
        cmd.extend(["-i", effect_path])
        filters.append(f"[{input_index}:a]adelay=900|900,volume=0.88[fx]")
        labels.append("[fx]")

    if len(labels) == 1:
        return tts_mp3_path

    filters.append("".join(labels) + f"amix=inputs={len(labels)}:duration=first:dropout_transition=2[aout]")
    cmd.extend([
...
def capture_html_animation_to_frames(html_path, frames_dir, seconds=ANIMATION_SECONDS, fps=ANIMATION_FPS):
    try:
        ...
        with sync_playwright() as p:
            ...
            for frame in range(total_frames):
                locator.screenshot(path=os.path.join(frames_dir, f"frame_{frame:04d}.png"))
...
def generate_video(html_path, audio_bytes, music_pool):
    ...
    mixed_audio = mix_audio_tracks(temp_tts, bgm, CARD_FLIP_SOUND, temp_mix)
    final_audio_path = mixed_audio if mixed_audio and os.path.exists(mixed_audio) else temp_tts

    frame_count = capture_html_animation_to_frames(html_path, frames_dir)
    if frame_count <= 0:
        print("❌ 动画帧捕获失败，无法生成视频")
    ...
    try:
        ...
        cmd = [
            ...
            "-b:a", "192k",
            "-pix_fmt", "yuv420p",
            "-vf", "scale=1280:720,format=yuv420p,tpad=stop_mode=clone:stop_duration=600",
            "-shortest",
...
def get_api_key(api_key_arg=None):
    api_key = api_key_arg or os.getenv("ARK_API_KEY") or os.getenv("VOLC_ARK_API_KEY")
    if not api_key:
...
def main():
    ...
    video_path = generate_video(html_path, audio_bytes, music_pool)

    if not video_path:
        print("\n【失败】视频合成失败，未生成最终 MP4。")
        return 1

    print(f"\n【完成】视频已生成：{os.path.abspath(video_path)}")
    return 0
