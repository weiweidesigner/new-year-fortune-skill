...
OPEN_SPEECH_RESOURCE_ID = os.getenv("OPEN_SPEECH_RESOURCE_ID", "seed-tts-2.0")
TTS_SPEAKER = os.getenv("OPEN_SPEECH_TTS_SPEAKER", "zh_female_vv_uranus_bigtts")
...
BGM_VOLUME = 0.18
WRITE_ANIMATION_FPS = int(os.getenv("WRITE_ANIMATION_FPS", "12"))  # 默认 12fps：兼顾书写效果和合成速度；想更丝滑可设为 18
...
if WRITE_FRAME_EXT not in {"jpg", "png"}:
    WRITE_FRAME_EXT = "jpg"
WRITE_FRAME_QUALITY = int(os.getenv("WRITE_FRAME_QUALITY", "88"))  # JPG 默认加速；想更清晰可设为 96
...
def http_post_bytes(url, data, headers, timeout=120):
    ...
    try:
        ...
        return raw, ctype or f"http-error/{e.code}"
...
def log_tts_non_audio_response(raw_text, content_type, resource_id, speaker, model):
    preview = re.sub(r"\s+", " ", raw_text or "").strip()[:1000]
    print("❌ OpenSpeech 返回了非音频结果")
    print(f"   Content-Type: {content_type or '(empty)'}")
...
def resolve_tts_resource_id(resource_id, speaker):
    resource_id = str(resource_id or "").strip() or "seed-tts-2.0"
    if str(speaker or "").endswith("_uranus_bigtts") and resource_id.startswith("volc.service_type."):
        return "seed-tts-2.0"
    return resource_id
...
def generate_tts_audio(fortune_data, tts_api_key=None):
    ...
    - OPEN_SPEECH_RESOURCE_ID：默认 seed-tts-2.0
    - OPEN_SPEECH_TTS_SPEAKER：音色，如 zh_female_*_bigtts
    - OPEN_SPEECH_TTS_MODEL：豆包语音大模型 2.0 模型名；为空则不传 model 字段
    """
    load_local_env()
    tts_api_key = tts_api_key or os.getenv("OPEN_SPEECH_X_API_KEY") or os.getenv("VOLC_TTS_API_KEY")
    ...
    resource_id = resolve_tts_resource_id(resource_id, speaker)
    ...
    try:
        ...
        audio = collect_streaming_audio(raw_text)
        if audio:
            return audio
        log_tts_non_audio_response(raw_text, content_type, resource_id, speaker, model)
        return None
    except Exception as e:
        print(f"❌ 语音合成失败：{e}")
        return None
