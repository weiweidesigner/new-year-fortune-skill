...
def generate_tts_audio(fortune_data, tts_api_key):
    if not tts_api_key:
        print("⚠️ 未配置 OPEN_SPEECH_X_API_KEY，跳过语音和视频生成")
        return None
...
def main():
    ...
    parser.add_argument("--output-format", type=str, default="both", choices=["html", "video", "both"], help="输出格式: html/video/both")
    ...
    if not audio_bytes:
        print("⚠️ 语音合成失败：无法生成视频")
        save_final_html(fortune, img_uri, False, "")
        return
