...
For GitHub publishing, keep a `.env` file out of the repository. A user can install and then configure secrets with their local skill manager, for example:

```bash
npx skills add https://github.com/<owner>/<repo> ARK_API_KEY=<ark-key> OPEN_SPEECH_X_API_KEY=<tts-key>
```
...
2. Run `scripts/happynewyear.py` from the skill folder.
3. Return the generated HTML path and, when available, the video path.
