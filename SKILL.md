...
- API密钥优先级:命令行参数 > 环境变量ARK_API_KEY / VOLC_ARK_API_KEY；不要把真实密钥写入技能代码
- 执行脚本时不要额外传入 `OPEN_SPEECH_TTS_SPEAKER`；脚本默认音色已经指定为 `zh_female_vv_uranus_bigtts`
- TTS 使用 OpenSpeech V3 豆包语音合成模型 2.0 调用结构：`X-Api-Key`、`X-Api-Resource-Id: seed-tts-2.0`、`X-Api-App-Key`、`X-Api-Request-Id`，请求体只包含 `user.uid` 与 `req_params`；不要发送旧的 `namespace: BidirectionalTTS`
- 视频捕获默认已做提速：`WRITE_ANIMATION_FPS=12`、`WRITE_FRAME_EXT=jpg`、`WRITE_FRAME_QUALITY=88`；如需更丝滑可临时调高这些环境变量
